...
grid = [map(int, x.strip().split(' ')) for x in grid] # Split in columns and convert to int using map

def greatest_product_in_matrix(matrix):
    largest = 0
    for i in matrix:
    ...
    for j in xrange(len(matrix)):
        ...
        for k in xrange(len(matrix)):
            multiple = multiple*matrix[k][k]
        if multiple > largest:
            largest = multiple
    multiple = 1
    right_dia = 1
    for k in xrange(len(matrix)):
        multiple = multiple*matrix[k][j]
        right_dia *= matrix[len(matrix)-1-k][k]
    if multiple > largest:
        largest = multiple
...
a = map(greatest_product_in_matrix,a)
